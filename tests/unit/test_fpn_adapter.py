...
@pytest.mark.parametrize("in_channels", [1536, 768])
...
def test_fpn_shape_mismatch_raises():
    adapter = FPNAdapter(in_channels=1536)
    tokens = torch.randn(1, 100, 1536)  # T=100 but spatial_shape says 64*64=4096
...
def test_fpn_param_count_reasonable():
    ...
    adapter = FPNAdapter(in_channels=1536, out_channels=256)
...
def test_fpn_works_with_batch_1():
    ...
    adapter = FPNAdapter(in_channels=1536)
    adapter.eval()
    tokens = torch.randn(1, 4096, 1536)
    with torch.no_grad():
