...
class _FakeBackbone(nn.Module):
    def __init__(self, summary_dim: int = 1152, spatial_dim: int = 1536):
        super().__init__()
...
def test_train_no_data_runs_config_only(monkeypatch, tmp_path: Path):
    ...
    class FakeClient:
        def __init__(self, registry_uri=None): pass
    ...
    run_id = train_detector(
        catalog="dev_cat", schema="dev_schema",
...
def test_precompute_image_loader(tmp_path: Path):
    ...
    from src.train.precompute_embeddings import _load_image_tensor
