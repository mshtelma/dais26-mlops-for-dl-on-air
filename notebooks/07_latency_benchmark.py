...
dbutils.widgets.text("endpoint", DETECTOR_ENDPOINT_NAME)
dbutils.widgets.text("num_requests", "1000")
dbutils.widgets.text("warmup_requests", "20")
dbutils.widgets.text("pivot_threshold_ms", "150")

endpoint = dbutils.widgets.get("endpoint")
num_requests = int(dbutils.widgets.get("num_requests"))
warmup_requests = int(dbutils.widgets.get("warmup_requests"))
pivot_threshold_ms = float(dbutils.widgets.get("pivot_threshold_ms"))
...
for _ in range(warmup_requests):
    try:
        w.serving_endpoints.query(name=endpoint, dataframe_split=payload)
    except Exception:
        pass
...
for i in range(num_requests):
    ...
    try:
        ...
        errors += 1
    if (i + 1) % 100 == 0:
        print(f"  done {i + 1}/{num_requests}; errors so far: {errors}")
...
if p99 > pivot_threshold_ms:
    print(f"\nPIVOT RECOMMENDED: p99 {p99:.1f}ms > threshold {pivot_threshold_ms}ms")
